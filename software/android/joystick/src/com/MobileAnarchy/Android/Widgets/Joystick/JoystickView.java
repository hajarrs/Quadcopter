...
public class JoystickView extends View 
{
	...
	private void initJoystickView() 
	{
		...
		// Circulo de fondo alrededor del joystick
		bgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		bgPaint.setColor(Color.LTGRAY);
		bgPaint.setStrokeWidth(1);
		bgPaint.setStyle(Paint.Style.FILL_AND_STROKE);

		// No se lo que es. Cambio el color y no hace nada...
		handlePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		handlePaint.setColor(Color.DKGRAY);
		handlePaint.setStrokeWidth(1);
		handlePaint.setStyle(Paint.Style.FILL_AND_STROKE);
			
		//Palo del joystick (union entre el boton y el centro
		...
		// Circulo pequeno del centro. (Oculto cuando el pulsador esta en el centro.
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) 
	{
		...
		bgRadius = dimX/2 - 2*innerPadding;
	...
	protected void onDraw(Canvas canvas) 
	{
		...
		handleX = touchX + cX;
		handleY = touchY + cY;
		canvas.drawCircle(cX, cY, handleRadius>>2, basePaint);
		canvas.drawLine(cX, cY, handleX, handleY, stickPaint);
		canvas.drawCircle(handleX, handleY, handleRadius/2, handlePaint);
